...
use azoth_core::cfg_ir::{
    BlockControlSnapshot, BlockModification, BlockSnapshotKind, CfgIrDiff, CfgIrSnapshot,
    JumpEncoding, JumpTargetSnapshot, OperationKind, TraceEvent, TraceJumpTargetKind,
};
...
fn build_jump_target_lookup(blocks: &[BlockInfo]) -> std::collections::HashMap<usize, Vec<usize>> {
    let mut lookup: std::collections::HashMap<usize, Vec<usize>> = std::collections::HashMap::new();
    for block in blocks {
        match &block.control {
...
fn format_annotated_bytecode(snap: &CfgIrSnapshot) -> Vec<Line<'static>> {
    let mut lines = Vec::new();
    ...
    let mut original_selectors: std::collections::HashSet<String> =
        std::collections::HashSet::new();
    let mut mapped_to_original: std::collections::HashMap<String, u32> =
        std::collections::HashMap::new();
    ...
    let mut preceded_by_fallthrough: std::collections::HashSet<usize> =
        std::collections::HashSet::new();
    // Also build a map of block node -> next block node (by PC order)
    let mut next_block_map: std::collections::HashMap<usize, usize> =
        std::collections::HashMap::new();
